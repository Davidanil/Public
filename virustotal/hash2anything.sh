...
##run command './hash2anything.sh' after previous steps being completed.
##Output file by default is result.txt but change it if you want. (But why would you?)
	...
	check
		...
		do	
			...
			do
				...
				echo "$noise" | grep -o '"sha1": "\w*"' >> tmp0
				echo "$noise" | grep -o '"result": "\w*",' | head -1 >> tmp0
				echo "$noise" | grep -o 'https:\/\/www.virustotal.com\/file\/\w*\/analysis\/\w*\/' >> tmp0
				echo "" >> tmp0
				n=$(( $n + 1 ))
